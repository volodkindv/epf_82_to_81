...
def Assemble(Dir, ModuleName):
        ...
        if ToWrite:
                # Module = Module.rstrip() + os.linesep #уберём пустые строки в конце файла
                if len(Module) > len(os.linesep):
                        Module = Module[:-len(os.linesep)]
...
def assemble_one(filename, deletesource=1):
        ...
        if not os.path.exists(fnModule):
                # тогда создадим пустой файл
                with open(fnModule, 'wb'):
                        pass
        # if deletesource == 1:
        #         os.remove(filename)

        # куда будем собирать готовый модуль? Его ведь потом еще залить внутрь epf надо будет.
        # может, отдельная директория?


def test():
        _target = r'c:\Git\source_1c_test\Модуль.1s'
